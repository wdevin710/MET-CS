...
corpus.txt[[1]]$content[2] # "Subject: Speaker's Info"
docs.tranf[[1]]$content[2] # "Subject: Speaker Info"
...
tfidf <- as.matrix(DocumentTermMatrix(Docs.corpus, control = list(weighting = weightTfIdf))) # TFIDF matrix
...
# Using The Jaccard similarity measures
library(stringdist)
dict1 <- "analytics"
dict2 <- "analysis"
query <- "analisis"
...
# Using The Sorensen-Dice similarity measure 
An <- length(A); Bn <- length(B) # Calculate the number of elements |A| & |B|

DA <- 2*(length(IA))/(An + Bn) 
sprintf("%s %f","The Sorensen-Dice Q-A measure is",DA)
# "The Sorensen-Dice Q-A measure is 0.588235"

DB <- 2*(length(IB))/(An + Bn) 
sprintf("%s %f","The Sorensen-Dice Q-B measure is",DB)
# "The Sorensen-Dice Q-B measure is 0.588235

# Using The Ochiai similarity measure 
OA <- (length(IA))/(An * Bn)^0.5 # 0.5892557
OB <- (length(IB))/(An * Bn)^0.5 # 0.5892557
